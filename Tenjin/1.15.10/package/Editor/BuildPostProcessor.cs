...
namespace TenjinSDK
{
    // @davitmk removed the inheritance from MonoBehaviour and made this class static.
    public static class BuildPostProcessor 
    {
        // @davitmk commented this out because Tenjin's SDK programmers are lazy and left this pointless method in the user's project. This shouldn't be here—it just adds an unnecessary menu item and clutters the user's project.
...
#if UNITY_IOS
    // @davitmk: Commented this out because, instead of manually extracting the ZIP file and adding it to the project, we’ll let Unity handle it since that’s the recommended approach.
    // @davitmk: To ensure everything works, we need to extract the framework from the ZIP file and configure the proper settings. This includes enabling "Add to Embedded Binaries" and making sure the required framework dependencies are enabled (AdServices.framework, AdSupport.framework, AppTrackingTransparency.framework, StoreKit.framework).
    // [PostProcessBuild(50)]
    // public static void OnPostProcessBuild(BuildTarget buildTarget, string path)
    // {
    //     if (buildTarget == BuildTarget.iOS)
    ...
    private static void UpdatePlist(string path)
    {
        ...
        // @davitmk added this line so users don't have to open and edit the value manually in Xcode.
